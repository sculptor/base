...
public class DomainObjectValidator<T> {
    ...
    public Map<String, ConstraintViolation<T>> getConstraintViolationsAsMap(
            T domainObject, Class<?>... groups) {
        return getConstraintViolationsAsMap(validator.validate(domainObject, groups));
    }

    public Set<ConstraintViolation<T>> getConstraintViolations(
            T domainObject, String propertyName, Class<?>... groups) {
        return validator.validateProperty(domainObject, propertyName, groups);
    }

    public Set<ConstraintViolation<T>> getConstraintViolations(
            Class<T> domainObject, String propertyName, Object value, Class<?>... groups) {
        return validator.validateValue(domainObject, propertyName, value, groups);
    }

    @SuppressWarnings("unchecked")
    public void validate(
            T domainObject, Class<?>... groups) {
	    Set constraintViolations = (Set) validator.validate(domainObject, groups);
    ...
    public void validateProperty(
            T domainObject, String propertyName, Class<?>... groups) {
        Set constraintViolations = (Set) validator.validateProperty(domainObject, propertyName, groups);
        if (constraintViolations != null && constraintViolations.size() > 0) {
            throw new ValidationException("", constraintViolations);
        }
    }
    ...
    public void validateValue(
            Class<T> domainObject, String propertyName, Object value, Class<?>... groups) {
        Set constraintViolations = (Set) validator.validateValue(domainObject, propertyName, value, groups);
        if (constraintViolations != null && constraintViolations.size() > 0) {
            throw new ValidationException("", constraintViolations);
        }
    }
     ...
     */
    @SuppressWarnings("unchecked")
    private Map<String, ConstraintViolation<T>> getConstraintViolationsAsMap(Set<ConstraintViolation<T>> violations)
    {
        Map<String, ConstraintViolation<T>> constraintViolationsMap = new HashMap<String, ConstraintViolation<T>>();
        for (ConstraintViolation constraintViolation : violations) {
            constraintViolationsMap.put(constraintViolation.getPropertyPath().toString(), constraintViolation);
        }
