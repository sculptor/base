    ...
    static abstract class AbstractQueryConverter implements QueryConverter {
        ...
        public String toString() {
            return String.format(
                "select=%1$s from=%2$s where=%3$s group by=%4$s order by=%5$s", select, from, where, groupBy, orderBy);
        }
    ...
    static class ConditionalCriteriaStrategy extends AbstractQueryConverter {
        ...
        private void convertSelections() {
            ...
            for (String selection : selections) {
                select += String.format(".select(%1$s)%2$s%3$s",
                        convertAttribute(attribute(selection)),
                        convertAlias(alias(selection)),
                        convertAggregate(aggregate(selection)));
            }
        }
        ...
        private void convertGroupBy() {
            ...
            for (String group : groups) {
                groupBy += String.format(".groupBy(%1$s)",
                        convertAttribute(attribute(group)));
            }
        }
        ...
        private void convertOrderBy() {
            ...
            for (String order : orders) {
                orderBy += String.format(".orderBy(%1$s)%2$s",
                        convertAttribute(attribute(order)),
        ...
        private void convertExpressions() {
            for (Expression expression : expressions) {
                ...
                while (matcher.find()) {
                    ...
                    addPlaceholder(
                        matcher.group(0).replaceAll(expression.getReplacement(), "").trim(),
