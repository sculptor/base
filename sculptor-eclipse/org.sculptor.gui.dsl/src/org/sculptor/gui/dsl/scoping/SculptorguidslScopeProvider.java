 ...
 * generated by Xtext
 */
package org.sculptor.gui.dsl.scoping;

import static org.sculptor.dsl.DslHelper.getExtends;
...
import org.sculptor.dsl.scoping.Scope;
...
import org.sculptor.gui.dsl.sculptorguidsl.DslCreateWith;
import org.sculptor.gui.dsl.sculptorguidsl.DslDeleteWith;
import org.sculptor.gui.dsl.sculptorguidsl.DslFindWith;
import org.sculptor.gui.dsl.sculptorguidsl.DslGuiListSubtask;
import org.sculptor.gui.dsl.sculptorguidsl.DslGuiListSubtaskForTarget;
import org.sculptor.gui.dsl.sculptorguidsl.DslGuiModule;
import org.sculptor.gui.dsl.sculptorguidsl.DslGuiPreviewProperty;
import org.sculptor.gui.dsl.sculptorguidsl.DslGuiViewProperty;
import org.sculptor.gui.dsl.sculptorguidsl.DslPopulateWith;
import org.sculptor.gui.dsl.sculptorguidsl.DslSearchWith;
import org.sculptor.gui.dsl.sculptorguidsl.DslSkipDomainObject;
import org.sculptor.gui.dsl.sculptorguidsl.DslSkipUserTask;
import org.sculptor.gui.dsl.sculptorguidsl.DslUpdateWith;
import org.sculptor.gui.dsl.sculptorguidsl.DslUserTask;
...
public class SculptorguidslScopeProvider extends AbstractDeclarativeScopeProvider {
    ...
    private void addDomainObjectsInModule(List<IEObjectDescription> elements, DslModule module) {
        for (DslSimpleDomainObject each : module.getDomainObjects()) {
            ...
            elements.add(new EObjectDescription(converter.toQualifiedName(each.getName()), each, null));
        }
    }
    ...
    private void populateProperties(List<IEObjectDescription> elements, DslSimpleDomainObject domainObject) {
        ...
        if (domainObject instanceof DslDomainObject) {
            for (DslProperty property : EcoreUtil2.typeSelect(domainObject.eContents(), DslProperty.class)) {
                elements.add(new EObjectDescription(converter.toQualifiedName(property.getName()), property, null));
            }
        ...
        if (domainObj instanceof DslDomainObject) {
            ...
            for (DslReference reference : references) {
                if (!(reference.getDomainObjectType() instanceof DslEnum)
                    ...
                    elements.add(new EObjectDescription(converter.toQualifiedName(reference.getName()), reference, null));
                }
            }
    ...
    IScope scope_DslGuiListSubtaskForTarget_target(DslGuiListSubtaskForTarget ctx, EReference ref) {
        ...
        elements.add(new EObjectDescription(converter.toQualifiedName(domainObject.getName()), domainObject, null));
        if (domainObject instanceof DslDomainObject) {
            ...
            for (DslDomainObject subclass : subclasses) {
                elements.add(new EObjectDescription(converter.toQualifiedName(subclass.getName()), subclass, null));
            }
        } else if (domainObject instanceof DslDataTransferObject) {
    ...
    IScope scope_DslCreateWith_operation(DslCreateWith ctx, EReference ref) {
        ...
        for (DslServiceOperation operation : operations) {
            elements.add(new EObjectDescription(converter.toQualifiedName(operation.getName()), operation, null));
        }
        return myScope;
    }
