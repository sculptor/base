...
public class JpaFlowExecutionListener extends FlowExecutionListenerAdapter {
	...
	public void sessionStarting(RequestContext context, FlowSession session, MutableAttributeMap input) {
	    if (!session.isRoot()) {
			...
			if (isPersistenceContext(parent.getDefinition())) {
				unbind(getEntityManager(context, parent.getDefinition().getId()));
			}
		}
		if (isPersistenceContext(session.getDefinition())) {
			EntityManager em = entityManagerFactory.createEntityManager();
            context.getConversationScope().put(session.getDefinition().getId() + "/" + PERSISTENCE_CONTEXT_ATTRIBUTE, em);
			bind(em);
		}
	}
	...
	public void paused(RequestContext context) {
		if (isPersistenceContext(context.getActiveFlow())) {
			unbind(getEntityManager(context, context.getActiveFlow().getId()));
		}
	}

	public void resuming(RequestContext context) {
		if (isPersistenceContext(context.getActiveFlow())) {
            bind(getEntityManager(context, context.getActiveFlow().getId()));
		}
	}

	public void sessionEnded(RequestContext context, FlowSession session, String outcome, AttributeMap output) {
		if (isPersistenceContext(session.getDefinition())) {
            final EntityManager em = (EntityManager) context.getConversationScope().remove(session.getDefinition().getId() + "/" + PERSISTENCE_CONTEXT_ATTRIBUTE);
    ...
    private EntityManager getEntityManager(RequestContext context, String flowId) {
        return (EntityManager) context.getConversationScope().get(flowId + "/" + PERSISTENCE_CONTEXT_ATTRIBUTE);
    }
